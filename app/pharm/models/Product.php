...
class ProductPh
{
    ...
    {
        $query = Connection::make()->prepare('SELECT products.*, countries.name as country, categories.name as category FROM products INNER JOIN categories ON products.category_id = categories.id WHERE products.id = :id');
        $query->execute(['id' => $id]);
