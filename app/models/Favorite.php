...
class Favorite
{
    ...
    {
        $query = Connection::make()->prepare("SELECT favorites.*, products.name as product_name, products.price as price, products.photo as photo FROM favorites INNER JOIN products ON product_id = products.id WHERE user_id = ?");
    ...
    public static function find($product_id, $user_id)
    {
        ...
        $query = Connection::make()->prepare("SELECT favorites.*, products.price as price FROM favorites INNER JOIN products ON product_id = products.id WHERE product_id = :product_id AND user_id = :user_id");
    ...
    public static function add($product_id, $user_id)
    {
        ...
        if (!$productInBasket) {
            $query = Connection::make()->prepare("INSERT INTO favorites (user_id, product_id) VALUES (:user_id, :product_id)");
    ...
    {
        $query = Connection::make()->prepare("DELETE FROM favorites WHERE product_id = :product_id AND user_id = :user_id");
        $query->execute(["user_id" => $user_id, "product_id" => $product_id]);
        return "delete";
    }
    ...
    {
        $query = Connection::make()->prepare("DELETE FROM favorites WHERE user_id = :user_id");
    ...
    public static function allDelete($user_id, $conn = null){
        ...
        $query = $conn->prepare("DELETE FROM favorites WHERE user_id = ?");
        $query->execute([$user_id]);
        return "deleteAll";
    }
}