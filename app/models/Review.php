...
class Review{
    ...
    public static function findUser($user_id){
        //получаем все отзывы у товара
        $query = Connection::make()->prepare("SELECT reviews.*, products.name AS product, users.name AS user, users.photo AS avatar FROM reviews INNER JOIN products ON reviews.product_id = products.id INNER JOIN users ON reviews.user_id = users.id WHERE user_id = :user_id");
