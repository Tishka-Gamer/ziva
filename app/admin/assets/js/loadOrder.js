document.addEventListener("DOMContentLoaded", () => {
  ...
  function checksChecked(arr) {
    ...
    async function getProductsByCategory(categories) {
        ...
        let products = await getData("/app/admin/tables/order/order.check.php", param);
    ...
    function getOneCard({order_id, name, status, date_start, date_end, pharmaceft_id}){
        ...
        <td class="table-td">${pharmaceft_id}</td>
        <td class="table-td"><form action="/app/admin/tables/order/status.php" method="post">
          <button name="id" value="${order_id}">Изменить статус</button>
          <input name="status_id" value="${status}" style="display: none;"></input>
        </form></td>
        <td class="table-td"><form action="/app/admin/tables/order/seeProd.php" method="post">
