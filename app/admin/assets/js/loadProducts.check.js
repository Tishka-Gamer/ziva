document.addEventListener("DOMContentLoaded", () => {
  ...
  function checksChecked(arr) {
    ...
    async function getProductsByCategory(type) {
        ...
        let products = await getData("/app/admin/tables/product/search.check.php", param);
    ...
    function getOneCard({id, name, experation_age, price, count, type, photo, relese}){
        ...
        <td class="table-td"><a class="atable" href="/app/admin/tables/product/red.php?id=${id}">Редактировать</a></td>
        <td class="table-td"><form action="/app/admin/tables/product/del.php" method="post"><button value="${photo}" name="btn">Удалить</button><input name="id" type="text" value="${id}" style="display: none;"></form></td>
        <td class="table-td"><form action="/app/admin/tables/product/show.php" method="post"><button name="btn">Подробнее</button><input name="id" type="text" value="${id}" style="display: none;"></form></td>
        </tr>`
    }
