...
class OrderAdm
{
    public static function allOrder(){
        $query = Connection::make()->query('SELECT DISTINCT orders.id as order_id, user_id, status, date_start, date_end, users.name as name, pharmaceft_id, products_in_order.count as count FROM orders INNER JOIN users ON orders.user_id = users.id INNER JOIN products_in_order ON products_in_order.order_id = orders.id ORDER BY date_start DESC');
    ...
    public static function editStatus($status,$id){
        $query = Connection::make()->prepare("UPDATE orders SET orders.status = :status WHERE orders.id = :id");
        $query->execute([
            "status" => $status,
            "id" => $id,
        ]);
